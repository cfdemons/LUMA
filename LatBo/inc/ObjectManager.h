...
public:
	...
	static void ibm_interpol(int ib, GridObj& g);			// Interpolation of velocity field on GridObj g onto markers of ib-th body
	static void ibm_spread(int ib, GridObj& g);			// Spreading of restoring force from ib-th body to GridObj g
	static void ibm_findsupport(int ib, 
		int m, GridObj& g);					// Populates support information for the m-th marker of ib-th body with support nodes 
														// on GridObj g.
	static void ibm_computeforce(int ib, GridObj& g);		// Compute restorative force at each marker in ib-th body using 
																	// charcteristic time scale of GridObj g.
	static double ibm_findepsilon(int ib, GridObj& g);		// Method to find epsilon weighting parameter for ib-th body given that the 
	...
	static void ibm_jacowire(int ib, GridObj& g);			// Computes the tension and position of a 2D inextensible, flexible iBody 
																	// filament and normailises lengths based on spacing on GridObj g.
	static void ibm_position_update(int ib, GridObj& g);	// Updates the position of deformable body markers and searches for support 
																	// on GridObj g.
	...
	static void io_restart(bool IO_flag, int level);	// Restart read and write for IBBodies given grid level
