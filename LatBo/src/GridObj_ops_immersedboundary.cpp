...
void GridObj::ibm_initialise() {
	...
	for (size_t ib = 0; ib < iBody.size(); ib++) {
		...
		bodyout.open(gUtils.path_str + "/IBbody_" + std::to_string(ib) + "_rank" + std::to_string(my_rank) + ".out");
		...
		for (size_t m = 0; m < iBody[ib].markers.size(); m++) {
			suppout.open(gUtils.path_str + "/Supp_" + std::to_string(ib) + "_" + std::to_string(m) + "_rank" + std::to_string(my_rank) + ".out");
		...
		std::ofstream epout;
		epout.open(gUtils.path_str + "/Epsilon_" + std::to_string(ib) + "_rank" + std::to_string(my_rank) + ".out");
		for (size_t m = 0; m < iBody[ib].markers.size(); m++) {
...
void GridObj::ibm_findsupport(unsigned int ib, unsigned int m) {
	...
	inear = (unsigned int)std::floor( iBody[ib].markers[m].position[0]/dx + 0.5);	// Simulates std::round
	jnear = (unsigned int)std::floor( iBody[ib].markers[m].position[1]/dy + 0.5);

#if (dims == 3)
	knear = (unsigned int)std::floor( iBody[ib].markers[m].position[2]/dz + 0.5);
#endif
...
double GridObj::ibm_findepsilon(unsigned int ib) {
	...
	// DEBUG -- write out A
	std::ofstream Aout;
	Aout.open(gUtils.path_str + "/Amatrix_" + std::to_string(ib) + "_rank" + std::to_string(my_rank) + ".out");
