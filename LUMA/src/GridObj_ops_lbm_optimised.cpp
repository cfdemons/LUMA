	...
	if (edgeCount > 1)
	{
...
#endif
		{
			if (type == ePressure)
			{
				...
				tmpVelVector[normalDirection] = GridUtils::extrapolate(*this, u_n, normalVector, 1, i, j, k, normalDirection, L_DIMS);
			}
		...
		if (type == ePressure)
		{
			...
			for (int d = 0; d < L_DIMS; d++)
			{
				...
				if (d != normalDirection && !GridUtils::isOnRecvLayer(XPos[i], YPos[j], ZPos[k]))
					tmpVelVector[d] = GridUtils::extrapolate(*this, u_n, normalVector, 1, i, j, k, d, L_DIMS);
			}
