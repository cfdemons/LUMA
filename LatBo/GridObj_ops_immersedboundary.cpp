...
// ***************************************************************************************************
// Method to initialise an IBbody
void GridObj::ibm_initialise() {

	// DEBUG -- write out marker coordinates
#ifdef IBM_DEBUG
	std::ofstream bodyout;
	bodyout.open("./Output/IBbody.out");
	bodyout << "x\ty\tz" << std::endl;
	for (size_t i = 0; i < iBody.markers.size(); i++) {
		bodyout << iBody.markers[i].position[0] << "\t" << iBody.markers[i].position[1] << "\t" << iBody.markers[i].position[2] << std::endl;
	}
	bodyout.close();
#endif
	...
	for (size_t m = 0; m < iBody.markers.size(); m++) {
		suppout.open("./Output/Supp_" + std::to_string(m) + ".out");
		suppout << "x\ty\tz" << std::endl;
		for (size_t i = 0; i < iBody.markers[m].supp_i.size(); i++) {
			if (dims == 3) {
				suppout << XPos[iBody.markers[m].supp_i[i]] << "\t" << YPos[iBody.markers[m].supp_j[i]] << "\t" << ZPos[iBody.markers[m].supp_k[i]] << std::endl;
			} else {
				suppout << XPos[iBody.markers[m].supp_i[i]] << "\t" << YPos[iBody.markers[m].supp_j[i]] << "\t" << ZPos[0] << std::endl;
			}
		}
		suppout.close();
	}
	...
	std::ofstream epout;
	epout.open("./Output/Epsilon.out");
	for (size_t m = 0; m < iBody.markers.size(); m++) {
		epout << iBody.markers[m].epsilon << std::endl;
	}
	epout.close();
#endif
...
void GridObj::ibm_findsupport(unsigned int m) {
	...
	if ( (int)inear - 5 < 0 || inear + 5 > XPos.size() ) {
		std::cout << "IB body is too near the X boundary of the grid so support cannot be guaranteed. Exiting." << std::endl;
		system("pause");
		exit(EXIT_FAILURE);
	
	} else if ( (int)jnear - 5 < 0 || jnear + 5 > YPos.size() ) {
		std::cout << "IB body is too near the Y boundary of the grid so support cannot be guaranteed. Exiting." << std::endl;
		system("pause");
		exit(EXIT_FAILURE);
		
	} else if ( (int)knear - 5 < 0 || knear + 5 > ZPos.size() ) {
		std::cout << "IB body is too near the Z boundary of the grid so support cannot be guaranteed. Exiting." << std::endl;
		system("pause");
		exit(EXIT_FAILURE);
	}
		...
		for (size_t j = jnear - 5; j <= jnear + 5; j++) {
			for (size_t k = knear - 5; k <= knear + 5; k++) {
				...
				if (	( abs(XPos[inear] - XPos[i])/dx < 1.5*iBody.markers[m].dilation ) &&
						...
						iBody.markers[m].supp_i.push_back(i);
						...
						iBody.markers[m].deltaval.push_back( delta_x * delta_y );
		...
		for (size_t i = 0; i < iBody.markers[m].deltaval.size(); i++) {
			...
			for (unsigned int dir = 0; dir < dims; dir++) {
				...
				iBody.markers[m].fluid_vel[dir] += u(	iBody.markers[m].supp_i[i],
													...
													dir,
													M_lim, K_lim, dims
													) * iBody.markers[m].deltaval[i] * iBody.markers[m].local_area;
#else
			iBody.markers[m].fluid_vel[dir] += u(	iBody.markers[m].supp_i[i],
													iBody.markers[m].supp_j[i],
													dir,
...
void GridObj::ibm_findepsilon() {
	...
	std::vector< std::vector<double> > A (iBody.markers.size(), std::vector<double>(iBody.markers.size(), 0.0) );

	// Loop over support of marker I and integrate delta value multiplied by delta value of marker J.
	for (size_t I = 0; I < iBody.markers.size(); I++) {

		// Loop over markers J
		for (size_t J = 0; J < iBody.markers.size(); J++) {

			// Sum delta values evaluated for each support of I
			for (size_t s = 0; s < iBody.markers[I].supp_i.size(); s++) {
	...
	// Create vectors
	std::vector<double> epsilon (iBody.markers.size(), 0.0);
	std::vector<double> bVector (iBody.markers.size(), 1.0);
	...
	for (size_t m = 0; m < iBody.markers.size(); m++) {
		iBody.markers[m].epsilon = epsilon[m];
	}
