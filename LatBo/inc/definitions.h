...
#define grav_force 1e-2	//( 3 * gUtils.vecnorm(u_0x,u_0y,u_0z) * nu / pow(fabs(b_y - a_y),2) )
...
// Global lattice indices for refined region on level L0 start numbering at 0
	...
	const static size_t RefYstart[NumReg]	= {10};
	const static size_t RefYend[NumReg]		= {20};
