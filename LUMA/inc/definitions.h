...
/// LUMA version
#define LUMA_VERSION "1.2.2-alpha"
...
#define L_N 640			///< Number of x lattice sites
...
#define L_u_ref 0.04	///< Reference velocity for scaling
#define L_u_max L_u_ref*1.5		///< Max velocity of inlet profile

// If not using an inlet profile, specify values or expressions here
#define L_u_0x 0.04			///< Initial/inlet x-velocity
...
#define L_SOLID_BLOCK_ON			///< Add solid block to the domain
	...
	#define L_block_x_min (L_N / 5)		///< Index of start of object/wall in x-direction
	#define L_block_x_max (L_block_x_min  + (L_N / 10))		///< Index of end of object/wall in x-direction
	#define L_block_y_min (L_block_x_min / 4)		///< Index of start of object/wall in y-direction
	#define L_block_y_max ((L_block_x_min / 4) + L_block_x_min / 2 )		///< Index of end of object/wall in y-direction
	#define L_block_z_min (L_block_x_min / 4)		///< Index of start of object/wall in z-direction
	#define L_block_z_max ((L_block_x_min / 4) + L_block_x_min / 2)		///< Index of end of object/wall in z-direction
...
// Following options are only here to making testing different grid combinations easier
	const static int RefXstart[L_NumLev][L_NumReg] = { { (L_N / 4) } };
	const static int RefXend[L_NumLev][L_NumReg] = { { (3 * L_N / 4) } };
	const static int RefYstart[L_NumLev][L_NumReg] = { { (L_N / 4) } };
	const static int RefYend[L_NumLev][L_NumReg] = { { (3 * L_N / 4) } };
	// If doing 2D, these can be arbitrary values
	static int RefZstart[L_NumLev][L_NumReg] = { { (L_N / 4) } };
	static int RefZend[L_NumLev][L_NumReg] = { { (3 * L_N / 4) } };
...
// Set dependent options
	...
	#else
		#define L_nVels 19		///< Number of lattice velocities
	#endif
