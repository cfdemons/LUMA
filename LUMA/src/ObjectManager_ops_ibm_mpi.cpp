...
void ObjectManager::ibm_buildMPIComms() {
	...
	for (int i = 0; i < mpim->num_ranks; i++) {
		for (int j = 0; j < nSupportSend[i]; j++) {

			// Unpack positions and body ID
			ib = bodyIDRecv[i][j];
			x = supportPositionsRecv[i][j*L_DIMS];
			y = supportPositionsRecv[i][j*L_DIMS+1];
			z = 0.0;
#if (L_DIMS == 3)
			z = supportPositionsRecv[i][j*L_DIMS+2];
#endif

			// Get indices of enclosing voxel
			GridUtils::getEnclosingVoxel(x, y, z, iBody[ib]._Owner, &nearijk);

			// Place in sending vector
			supportCommSupportSide.emplace_back(i, ib, nearijk);
		}
	}
}
